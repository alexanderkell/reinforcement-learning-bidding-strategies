...
%DIF DEL previous_version.tex    Sat Nov 14 15:53:44 2020
%DIF ADD conference_101719.tex   Sat Nov 14 15:34:25 2020
...
%DIF < \thanks{EPSRC}
\DIFaddbegin \thanks{EPSRC}
\DIFaddend }

\author{\DIFdelbegin %DIFDELCMD < \IEEEauthorblockN{Alexander J. M. Kell, Matthew Forshaw, A. Stephen McGough}
%DIFDELCMD < \IEEEauthorblockA{\textit{School of Computing} \\
%DIFDELCMD < \textit{Newcastle University}\\
%DIFDELCMD < Newcastle upon Tyne, U.K. \\
%DIFDELCMD < \{a.kell2, matthew.forshaw, stephen.mcgough\}@newcastle.ac.uk}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \IEEEauthorblockN{Alexander J. M. Kell\textsuperscript{\textsection}}
\IEEEauthorblockA{\textit{Department of Chemical Engineering} \\
\textit{Imperial College London}\\
London, U.K. \\
a.kell@imperial.ac.uk}
\and
\IEEEauthorblockN{Matthew Forshaw, A. Stephen McGough}
\IEEEauthorblockA{\textit{School of Computing} \\
\textit{Newcastle University}\\
Newcastle \\
\{matthew.forshaw, stephen.mcgough\}@newcastle.ac.uk}
}
\DIFaddend %\and
%DIF < \IEEEauthorblockN{2\textsuperscript{nd} Given Name Surname}
%DIF < \IEEEauthorblockA{\textit{dept. name of organization (of Aff.)} \\
%DIF < \textit{name of organization (of Aff.)}\\
%DIF < City, Country \\
...
\footnotetext{\DIFadd{This work was carried out whilst at Newcastle University.}}
\endgroup
\DIFaddend 
...
Simulation is often used as an environment for RL to learn and act in. This is due to the ability for simulation to perform actions in faster than real-time. In this section we explore \DIFdelbegin \DIFdel{reinforcement learning }\DIFdelend \DIFaddbegin \DIFadd{RL }\DIFaddend approaches used to make intelligent bidding decisions in electricity markets. RL is a suitable method for analyzing the dynamic behaviour of complex systems with uncertainties. RL can, therefore be used to identify optimal bidding strategies in energy markets \cite{Yang2020}. In the following papers simulations are used as the environment.
...
This is the policy gradient, the gradient of the policy's performance. The policy gradient method optimizes the policy directly by updating the weights, $\theta$, in such a way that an optimal policy is found within finite time. This is achieved by performing gradient ascent on the policy and \DIFdelbegin \DIFdel{its }\DIFdelend parameters $\pi^\theta$.
...
Figure \ref{fig:model_details} shows how the six components interact. The electricity demand is matched with the supply of the power plants, owned by the \DIFdelbegin \DIFdel{generator companies (GenCos)}\DIFdelend \DIFaddbegin \DIFadd{GenCos}\DIFaddend . We have configured ElecSim to model the UK electricity market, using the configuration file. Specifically, we model the actual GenCos with their respective power plants that were in operation in 2018.



\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{figures/methedology/system-overview-v2}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \centering
    \includegraphics[width=0.45\textwidth]{figures/methedology/system-overview-v2}
    \DIFaddendFL \caption{System overview of ElecSim \cite{Kell}.}
...
Much of the data used by the algorithm was generated by the simulation and persisted in memory. With over 4,000,000 bids made by all of the power plants in each simulation run. Each bid had \DIFdelbegin \DIFdel{six different state data and a }\DIFdelend \DIFaddbegin \DIFadd{a state space size of six and a single }\DIFaddend reward; leading to 28,000,000 data points per simulation. 1,000 simulation runs, therefore leads to $28\times 10^9$ data points used to train the reinforcement learning algorithm.

By modelling bidding decisions as a RL algorithm, we hope to observe the ability for RL to find the point at which market power artificially inflates electricity prices. To achieve this, we choose the six largest GenCos in the UK, as well as a smaller GenCo as a control. Groups of GenCos are modelled as a single GenCo with a single RL strategy for the purpose of this paper. \DIFaddbegin \DIFadd{These replace the case where bids are made based on the SRMC. }\DIFaddend Table \ref{table:genco_table} displays the groups of GenCos, as well as individual GenCos, with their respective capacity and number of plants.


\DIFaddbegin \DIFadd{The only action of the RL agents is to make a bid for each of their power plants at each time-step. Over time, the RL algorithm learns an optimal bidding strategy for their power plants, to maximise the amount of money they receive from the market. It is posited that the dynamics of the market change with respect to the amount of electricity generation capacity owned by the RL agents. For example, RL agents which bid into a competitive market must bid the lowest price. However, RL agents which bid into an uncompetitive market can artificially increase the price they bid. 
}

\DIFaddend \begin{table*}
\DIFdelbeginFL %DIFDELCMD < \renewcommand{\arraystretch}{1.35}
%DIFDELCMD < %%%
...
For the reinforcement learning problem we have the following tuple: $(s_t,a_t,r_t,s_{t+1})$, where $(s_t, s_{t+1})$ is the state at time $t$ and $t+1$ respectively, $a_t$ is the action at time $t$\DIFaddbegin \DIFadd{, or the bid made into the market, }\DIFaddend and $r_t$ is the reward at time $t$. For our problem the state space\DIFaddbegin \DIFadd{, which are the observations given to the RL agents following each action, or bid, }\DIFaddend is given by the tuple shown in Equation \ref{eq:observation_tuple}\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{. These observations allow the RL agents to learn how their bids should relate to this information about the market.
}\DIFaddend \begin{equation}
\label{eq:observation_tuple}
	s_t=(H_i,D_i,p_{CO_{2}},p_{gas},p_{coal},p_{c}),
\end{equation}
\noindent where $H_i$ is the segment hour to bid into at timestep $i$, $D_i$ is the demand of the segment hour at timestep $t$, $p_{gas}$ is the price of gas, $p_{coal}$ the price of coal, $p_{CO_{2}}$ is the carbon tax price, and $p_{c}$ is the clearing price. We set the reward, $r_t$\DIFaddbegin \DIFadd{, }\DIFaddend to be the average electricity price of that time step, $p_{avg}$, as shown by Equation \ref{eq:reward}:
...
\noindent \DIFdelbegin \DIFdel{where }\DIFdelend $p_m$ is the price for the $m$ load segment for the respective time step, and $M$ is the total load segments\DIFdelbegin \DIFdel{, which in our case is }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend 20. 
...
Figures \ref{fig:unbounded_timesteps} and \ref{fig:bounded_timesteps} show the rewards over a number of time steps for the unbounded and bounded cases respectively. Figure \ref{fig:unbounded_timesteps} shows a clear difference between agents which use the DDPG RL strategy and have a large capacity (green and yellow) compared to those which have a smaller capacity (dark purple). The axis in Figure \ref{fig:unbounded_timesteps} are much larger than those of \ref{fig:bounded_timesteps}, highlighting the effect of market power\DIFdelbegin \DIFdel{on an unbounded market}\DIFdelend .


\begin{figure}
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{figures/results/unbounded_results.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.45\textwidth]{figures/results/unbounded_results.pdf}
...
\begin{figure}
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{figures/results/bounded_results.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.45\textwidth]{figures/results/bounded_results.pdf}
...
On the other hand, once the capacity of a GenCo or groups of GenCos is above 30,000MW, there is a significant increase in the average price for capacity. The average electricity price for capacity falls between \textsterling$170$/MWh and \textsterling$220$/MWh. 
...
\begin{figure}[]
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{figures/results/unbounded_results_scatter.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.45\textwidth]{figures/results/unbounded_results_scatter.pdf}
...
\DIFaddbegin \DIFadd{It is noted that in the competitive market scenarios, the electricity price is close to the actual electricity price in the UK. Therefore, we can have confidence in the behaviour of our model and the RL strategy.
}
...
\DIFaddend \begin{figure}
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{figures/results/bounded_results_scatter.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.45\textwidth]{figures/results/bounded_results_scatter.pdf}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[b]{0.45\textwidth}   
...
\begin{figure}
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{figures/results/ecdf_all.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.45\textwidth]{figures/results/ecdf_all.png}
    \DIFaddendFL \caption{Empirical cumulative distribution function comparing the likelihood of each bid for the different sized GenCos and market cap.}
    \label{fig:ecdf_all}
\end{figure}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{figures/results/sensitivity_analysis.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \centering
    \includegraphics[width=0.45\textwidth]{figures/results/sensitivity_analysis.png}
    \DIFaddendFL \caption{Capacity of agents using RL vs. average electricity price accepted, for bounded agents.}
...
\DIFaddbegin \DIFadd{We show that in the competitive market case, our RL agents bid a price similar to the average electricity price in the UK. This gives us confidence in the strategy of the RL algorithm. 
}

\DIFaddend Our work has shown the ability for reinforcement learning to learn an optimal bidding strategy to maximize a GenCo's profit within an electricity market. The ability for GenCos to use their market power is also highlighted, and is dependent on electricity generation capacity of the respective GenCo.
...
In future work, we would like to enable GenCos to withhold the capacity on offer to the electricity market. This would enable further market power by reducing competition further.  Additionally, we would like to assess the market power in different countries with different market structures and total electricity supply\DIFaddbegin \DIFadd{. In addition, we would like to run the full long-term model by using parallel computing, to see the long-term effect of market power on the market}\DIFaddend . 
