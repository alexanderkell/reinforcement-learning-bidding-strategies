...
%DIF DEL original_submitted_version.tex   Tue Aug 11 11:24:47 2020
%DIF ADD conference_101719.tex            Tue Aug 11 08:07:11 2020
...
Decentralized electricity markets are often dominated by a small set of generator companies who control the majority of the capacity. In this paper, we explore the effect of the total controlled electricity capacity by any single\DIFaddbegin \DIFadd{, }\DIFaddend or group of generator companies\DIFaddbegin \DIFadd{, }\DIFaddend on the average electricity price. We demonstrate this through \DIFaddbegin \DIFadd{an application with }\DIFaddend ElecSim, a simulation of a country-wide energy market. We develop an agent, representing a \DIFdelbegin \DIFdel{power generator }\DIFdelend \DIFaddbegin \DIFadd{generation company, }\DIFaddend which uses a deep deterministic policy \DIFaddbegin \DIFadd{gradient }\DIFaddend reinforcement learning algorithm to bid in a uniform pricing electricity market strategically. A uniform pricing market is one where all players are paid the highest accepted price. ElecSim is parameterized to the United Kingdom for the year 2018. This work can help inform policy on how to best regulate a market to ensure that the price of electricity remains competitive.
...
\cite{Hunt2016a}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . To solve this, an actor-critic approach based on the deterministic policy gradient (DPG) algorithm is used \cite{Silver2014}.
...
\end{equation}
\noindent where $H_i$ is the segment hour to bid into at timestep $i$, $D_i$ is the demand of the segment hour at timestep $t$, $p_{gas}$ is the price of gas, $p_{coal}$ the price of coal, $p_{C02}$ is the carbon tax price, and $p_{c}$ is the clearing price. We set the reward, $r_t$ to be the average electricity price of that time step, $p_{avg}$\DIFdelbegin \DIFdel{.
