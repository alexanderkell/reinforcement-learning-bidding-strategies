...
%DIF DEL original_submitted_version.tex   Mon Aug 17 09:56:34 2020
%DIF ADD conference_101719.tex            Tue Aug 25 18:15:33 2020
...
Decentralized electricity markets are often dominated by a small set of generator companies who control the majority of the capacity. In this paper, we explore the effect of the total controlled electricity capacity by \DIFdelbegin \DIFdel{any singleor group }\DIFdelend \DIFaddbegin \DIFadd{a single, or group, }\DIFaddend of generator companies \DIFaddbegin \DIFadd{can have }\DIFaddend on the average electricity price. We demonstrate this through \DIFaddbegin \DIFadd{the use of }\DIFaddend ElecSim, a simulation of a country-wide energy market. We develop \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a strategic }\DIFaddend agent, representing a \DIFdelbegin \DIFdel{power generator }\DIFdelend \DIFaddbegin \DIFadd{generation company, }\DIFaddend which uses a deep deterministic policy \DIFaddbegin \DIFadd{gradient }\DIFaddend reinforcement learning algorithm to bid in a uniform pricing electricity market\DIFdelbegin \DIFdel{strategically}\DIFdelend . A uniform pricing market is one where all players are paid the highest accepted price. ElecSim is \DIFdelbegin \DIFdel{parametrized }\DIFdelend \DIFaddbegin \DIFadd{parameterized }\DIFaddend to the United Kingdom for the year 2018. This work can help inform policy on how to best regulate a market to ensure that the price of electricity remains competitive.

We find that capacity has an impact on the average electricity price in a single year. If any single generator company, or a collaborating group of generator companies, control more than ${\sim}$11$\%$ of generation capacity \DIFaddbegin \DIFadd{and bid strategically}\DIFaddend , prices begin to increase by ${\sim}$25$\%$. The value of ${\sim}$25\% and ${\sim}$11\% may vary between market structures and countries. For instance, different load profiles may favour a particular type of generator or a different distribution of generation capacity. Once the capacity controlled by a generator company\DIFaddbegin \DIFadd{, which bids strategically, }\DIFaddend is higher than ${\sim}$35\%\DIFdelbegin \DIFdel{of the total capacity}\DIFdelend , prices increase exponentially. \DIFdelbegin \DIFdel{The use of }\DIFdelend \DIFaddbegin \DIFadd{We observe that the use of the }\DIFaddend a market cap of approximately double the average market price has the effect of significantly decreasing this effect and maintaining a competitive market. %DIF <  A fair and competitive electricity market provides value to consumers and enables a more competitive economy through the utilisation of electricity by both industry and consumers.
\DIFaddbegin \DIFadd{A fair and competitive electricity market provides value to consumers and enables a more competitive economy through the utilisation of electricity by both industry and consumers.
}\DIFaddend 
...
Reduced competition within electricity markets can lead to higher prices to consumers, with no societal benefit. It is, therefore, within the interests of the consumer and that of government to maintain a competitive market. Low energy costs enable innovation in other industries reliant on electricity, and in turn, make for a more productive economy. %DIF <  Competition within electricity markets can be increased by only allowing single entities to have control over a small amount of electricity generation capacity.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In this paper, we explore \DIFaddbegin \DIFadd{how to increase competition within electricity markets.
}

\DIFadd{We explore }\DIFaddend the effect of total control over capacity on electricity prices. Specifically, we model different sizes of GenCos and groups of colluding GenCos, \DIFdelbegin \DIFdel{to bid }\DIFdelend \DIFaddbegin \DIFadd{bidding }\DIFaddend strategically to maximize their profit using a \DIFdelbegin \DIFdel{reinforcement }\DIFdelend learning algorithm. This is in contrast to the strategy of bidding using the SRMC of their respective power plants, which would occur under perfect market \DIFdelbegin \DIFdel{conditions. }\DIFdelend \DIFaddbegin \DIFadd{competition. Perfect market competition is a theoretical market structure where all firms sell an identical product, all firms are price-takers and market share has no influence on price. }\DIFaddend To model this we use \DIFaddbegin \DIFadd{an application of }\DIFaddend deep reinforcement learning (RL) to calculate a bidding strategy for GenCos in a day-ahead market. These GenCos are modelled as agents within the agent-based model, ElecSim \cite{Kell, Kell2020}. We use the UK electricity market instantiated on 2018 as a case study, similar to our work in \cite{Kell2019a}. That is, we model each GenCo with their respective power plants in the year 2018 to 2019. In total, we model 60 GenCos with \DIFdelbegin \DIFdel{1085 }\DIFdelend \DIFaddbegin \DIFadd{1,085 }\DIFaddend power plants. It is possible, however, to generalise this approach and model to any other decentralized electricity market. 
...
\DIFaddbegin \DIFadd{The contributions of this work are to use a novel agent-based model to explore market power in electricity markets using a deep reinforcement learning algorithm. Oligopolistic markets often exhibit market power which can lead to artificially inflated electricity prices. We find an optimal market cap level in the UK electricity market as well as the maximum proportion of capacity that a single entity should own to prevent imperfect market conditions.
}

\DIFaddend In Section \ref{sec:lit-review} we review the literature, and explore other uses of RL in electricity markets. In Section \ref{sec:material} we introduce the agent-based model used and the DDPG algorithm. Section \ref{sec:methodology} explores the methodology taken for our case study. The results are presented in Section \ref{sec:results}. We discuss and conclude our work in Sections \ref{sec:discussion} and \ref{sec:conclusion} respectively. 
...
Intelligent bidding strategies for day-ahead electricity markets can be divided into two broad categories: game-theoretic models and simulation. \DIFaddbegin \DIFadd{Game-theoretic approaches may struggle in complex electricity markets where Nash equilibriums do not exist \mbox{%DIFAUXCMD
\cite{Wang2011}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Agent-based models (ABMs) allow for the simulation of heterogenous irrational actors with imperfect information. Additionally, ABMs allow for learning and adaption within a dynamic environment \cite{EsmaeiliAliabadi2017}.
\DIFdelbegin \DIFdel{Game-theoretic approaches may struggle in complex electricity markets where Nash equilibriums do not exist \mbox{%DIFAUXCMD
\cite{Wang2011}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend 
...
\subsection{\DIFdelbegin \DIFdel{Reinforcement learning to model intelligence}\DIFdelend \DIFaddbegin \DIFadd{Simulation}\DIFaddend }

\DIFdelbegin \DIFdel{Next, }\DIFdelend \DIFaddbegin \DIFadd{Simulations are a software system which can replicate the salient features of a real-world system. Through the modelling of such systems using simulation, `what-if' analysis can be undertaken. These `what-if' analysis can be used to gain a greater understanding how a system would behave under a change in environment or policy. Simulation is especially useful when performing changes to the real system would be unpalatable to perform - such as having a significant impact or having a high cost. 
}

\DIFadd{`Digital twins' are a specific instance of a simulation. For example, a simulation can be built of a decentralized electricity market, whereas a digital twin would be a simulation of the UK electricity market. A digital twin allows for rapid experimentation to be carried out, in faster than real-time, and avoid the risk that this experimentation could have. 
}

\DIFadd{Simulations are often used to understand markets and how an individual or entity should optimally behave. This is due to the fact that there is a high risk involved in submitting bids due to the potential negative effects of having a bid rejected. In addition to this, the uncertainties in a market can be better understood through monte-carlo simulation, by exploring various scenarios.
}


\subsection{\DIFadd{Reinforcement learning to model intelligence}}

\DIFadd{Simulation is often used as an environment for RL to learn and act in. This is due to the ability for simulation to perform actions in faster than real-time. In this section }\DIFaddend we explore reinforcement learning approaches used to make intelligent bidding decisions in electricity markets. RL is a suitable method for analyzing the dynamic behaviour of complex systems with uncertainties. RL can, therefore be used to identify optimal bidding strategies in energy markets \cite{Yang2020}. \DIFdelbegin \DIFdel{Simulations are often used to provide an environment for the reinforcement learning algorithm. }\DIFdelend In the following papers simulations are used as the environment.


Aliabadi \textit{et al.} utilize an ABM and the Q-learning algorithm to study the impact of learning and risk aversion on GenCos in an oligopolistic electricity market with five GenCos \cite{EsmaeiliAliabadi2017}. They find that some level of risk aversion is beneficial, however excessive risk degrades profits by causing an intense price competition. Our paper focuses on the impact of the interaction of \DIFdelbegin \DIFdel{many }\DIFdelend \DIFaddbegin \DIFadd{60 }\DIFaddend GenCos within the UK electricity market. In addition, we extend the Q-learning algorithm to use the DDPG algorithm, which uses a continuous action space. \DIFaddbegin \DIFadd{A continuous action space allows for more granular action spaces to be taken, allowing for a more precise action. In addition, by modelling 60 GenCos, we are able to more closely match the real-life dynamics of the UK electricity market. 
}\DIFaddend 
...
\subsection{\DIFadd{Deep Deterministic Gradient Policy}}


\DIFaddend Many problems are naturally discretized which are well suited to a Q-learning approach, however this is not always the case\DIFdelbegin \DIFdel{. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Deep Deterministic Gradient Policy}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{, such as the problem in this paper. }\DIFaddend It is not straightforward to apply Q-learning to continuous action spaces. This is because in continuous spaces, finding the greedy policy requires an optimization of $a_t$ at every time step. Optimizing for $a_t$ at every time step would be too slow to be practical with large, unconstrained function approximators and nontrivial action spaces \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hunt2016a}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Hunt2016a}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . To solve this, an actor-critic approach based on the deterministic policy gradient (DPG) algorithm is used \cite{Silver2014}. \DIFaddbegin \DIFadd{This approach has not often been taken in the literature, and therefore is a novelty of our approach.
}\DIFaddend 


The DPG algorithm maintains a parameterized actor function $\mu(s|\theta^\mu)$ which specifies the current policy by deterministically mapping states to a specific action. The critic $Q(s,a)$ is learned using the Bellman equation\DIFaddbegin \DIFadd{, }\DIFaddend as in Q-learning. The actor is updated by applying the chain rule to the expected return from the start distribution $J$ with respect to the actor parameters:
\begin{align}
\DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD < 	\triangledown_{\theta^\mu}J\approx\mathbb{E}_{s_t\sim\rho^\beta}[\triangledown_{\theta^\mu}Q(s,a|\theta^Q)|_{s=s_t,a=\mu(s_t|\theta^\mu)}] \\
%DIFDELCMD < 	= \mathbb{E}_{s_t\sim\rho^\beta}[\triangledown_aQ(s,a|\theta^Q)|_{s=s_t,a=\mu(s_t)}\triangledown_{\theta_\mu}\mu(s|\theta^\mu)|_{s=s_t}]
%DIFDELCMD < \end{split}%%%
...
%DIFDELCMD <   %%%
\DIFdelend %DIF >   \footnotesize
  \DIFaddbegin \small
  \DIFaddend \STATE Initialize critic network $Q(s,a|\theta^Q)$ and actor $\mu(s|\theta^\mu)$ with random weights $\theta^Q$ and $\theta^\mu$
...
We utilized the long-term electricity market agent-based model, ElecSim \cite{Kell,Kell2020}. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{ElecSim is a simulation which consists of multiple generation companies which interact in a simulated electricity market. This electricity market is able to run for 20+ years and can show what long-term scenarios may develop from certain scenario configurations, such as high carbon and fuel prices. The generation companies invest in power plants based upon their understanding and expectations of market conditions now and in the future. They then sell the electricity generators on a day-ahead spot market. The }\DIFaddend model was run using a short term approach by only iterating through a single year (2018), composed of eight representative days, each of 24 time steps.
...
For the purpose of this work, we do not consider flow constraints within the electricity mix. This is because we model the entire UK with 1000+ generators, and many nodes and buses. This would make the optimization problem intractable for the purpose of our simulation, especially when considering the many episodes required for training. It takes ${\sim}125$ seconds to run \DIFaddbegin \DIFadd{each episode (representing }\DIFaddend a single year in the simulation\DIFdelbegin \DIFdel{, or episode }\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend with our current setup. By increasing the simulation time further, we would make the compute time intractable\DIFdelbegin \DIFdel{due to the many episodes required for reinforcement learning to learn an effective policy}\DIFdelend . Additionally, we must train several reinforcement learning policies to account for each GenCo and market cap. Therefore a simulation that takes ${\sim}$2 minutes to run, \DIFdelbegin \DIFdel{run }\DIFdelend \DIFaddbegin \DIFadd{repeated }\DIFaddend 1000 times takes ${\sim}$33 hours \DIFdelbegin \DIFdel{to run}\DIFdelend \DIFaddbegin \DIFadd{in total to finish}\DIFaddend , multiplied by 12 different GenCos takes ${\sim}$16 days, and with two market cap scenarios it would take ${\sim}$1 month.
...
\DIFaddbegin \subsection{\DIFadd{Model Parametrization}}

\DIFaddend To parameterize the simulation, we use data from the United Kingdom in 2018. This included 1085 electricity generators and power plants with their respective GenCos. The data for this was taken from the BEIS DUKES dataset \cite{dukes_511}. The \DIFaddbegin \DIFadd{data included construction costs, capital and operating costs as well as lifetimes of the plant and capacity. The storage size of all of these power plants was 10MB of data. The }\DIFaddend electricity load data was modelled using data from \cite{gridwatch}; offshore and onshore wind and solar irradiance data from \cite{Pfenninger2016} \DIFaddbegin \DIFadd{and are 20MB in size}\DIFaddend . It would be possible to adopt this approach to other decentralized markets in other countries.


\DIFaddbegin \subsection{\DIFadd{Reinforcement Learning Setup}}

\DIFadd{Much of the data used by the algorithm was generated by the simulation and persisted in memory. With over 4,000,000 bids made by all of the power plants in each simulation run. Each bid had six different state data and a reward; leading to 28,000,000 data points per simulation. 1,000 simulation runs, therefore leads to $28\times 10^9$ data points used to train the reinforcement learning algorithm.
}

\DIFaddend By modelling bidding decisions as a RL algorithm, we \DIFdelbegin \DIFdel{hoped }\DIFdelend \DIFaddbegin \DIFadd{hope }\DIFaddend to observe the ability for RL to find the point at which market power artificially inflates electricity prices. To achieve this, we \DIFdelbegin \DIFdel{chose }\DIFdelend \DIFaddbegin \DIFadd{choose }\DIFaddend the six largest GenCos in the UK, as well as a smaller GenCo as a control. Groups of GenCos are modelled as a single GenCo with a single RL strategy for the purpose of this paper. Table \ref{table:genco_table} displays the groups of GenCos, as well as individual GenCos, with their respective capacity and number of plants.
...
\DIFadd{\label{eq:reward}
	r_t=p_{avg},
}\end{equation}

\noindent \DIFadd{where $p_{avg}$ is defined in Equation \ref{eq:p_avg}:
}

\begin{equation}
\DIFadd{\label{eq:p_avg}
	p_{avg}=\frac{\sum_{m=0}^{M}p_m}{M},
}\end{equation}

\noindent \DIFadd{where $p_m$ is the price for the $m$ load segment for the respective time step, and $M$ is the total load segments, which in our case is 20. 
}

\DIFadd{This reward function allowed us to closely match the profit that would be made by the GenCo. For instance, if a bid was not accepted, the GenCo would receive \textsterling0. If the bid was accepted, however, the total money received would be proportional to the reward, giving the GenCo an incentive to learn the dynamics of the electricity market clearing process. 
}

\DIFadd{We set the replay buffer size, $\mathcal{R}$, to be 50,000. This enabled us to store a relatively large number of history, whilst maintaining tractability.
}

\DIFaddend For the action space, $a_t$, we modelled two scenarios\DIFdelbegin \DIFdel{. Where }\DIFdelend \DIFaddbegin \DIFadd{, as shown by Equations \ref{eq:action-space-scen-1} and \ref{eq:action-space-scen-2}:
}

\begin{equation}
\DIFadd{\label{eq:action-space-scen-1}
	0 \leq a_t \leq 600,
}\end{equation}

\begin{equation}
\DIFadd{\label{eq:action-space-scen-2}
	0 \leq a_t \leq 150,
}\end{equation}


\noindent \DIFadd{where }\DIFaddend there was a price cap of \textsterling\DIFdelbegin \DIFdel{$150$}\DIFdelend \DIFaddbegin \DIFadd{$600$}\DIFaddend /MWh \DIFdelbegin \DIFdel{and \textsterling$600$}\DIFdelend \DIFaddbegin \DIFadd{for Equation \ref{eq:action-space-scen-1} and \textsterling$150$}\DIFaddend /MWh \DIFdelbegin \DIFdel{. Only these }\DIFdelend \DIFaddbegin \DIFadd{for Equation \ref{eq:action-space-scen-2}. Only }\DIFaddend two values were chosen to reduce computational load. We chose \textsterling$150$/MWh as a reasonable price cap that may be introduced by a Government. This was roughly double the average accepted price in 2018, therefore allowed for higher prices in times of high demand or low supply. The \DIFdelbegin \DIFdel{$\textsterling 600$}\DIFdelend \DIFaddbegin \DIFadd{\textsterling 600}\DIFaddend /MWh was chosen to simulate an unbounded price cap. This enabled us to see the price that an equilibrium is reached within a market with agents with market power.  



\DIFdelbegin \DIFdel{For this work, we assume that the action space $a_t$ only bids price, and not how much capacity to bid on the market. We assume this to reduce the dimensionality of $a_t$, and simplify the training process.
}\DIFdelend %DIF > For this work, we assume that the action space $a_t$ affects only bids price, and not how much capacity to bid on the market. We assume this to reduce the dimensionality of $a_t$, and simplify the training process.
...
\begin{figure}
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.40\textwidth]{figures/results/unbounded_results.pdf}
%DIFDELCMD <     %%%
...
\begin{figure}
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.40\textwidth]{figures/results/bounded_results.pdf}
%DIFDELCMD <     %%%
...
\begin{figure}[]
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.40\textwidth]{figures/results/unbounded_results_scatter.pdf}
%DIFDELCMD <     %%%
...
\begin{figure}
	\centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.40\textwidth]{figures/results/bounded_results_scatter.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.49\textwidth]{figures/results/bounded_results_scatter.pdf}
    \DIFaddendFL \caption{Capacity of agents using RL vs. average electricity price accepted, for bounded agents.}
    \label{fig:bounded_results_scatter}
\end{figure}



Figures \ref{fig:capped_600_bids} and \ref{fig:capped_150_bids} display the \DIFdelbegin \DIFdel{actual bids made }\DIFdelend \DIFaddbegin \DIFadd{bids which use the RL algorithm for every power plant }\DIFaddend at the end of training\DIFdelbegin \DIFdel{within the electricity market for all of their power plants. The }\DIFdelend \DIFaddbegin \DIFadd{. For instance, in Figure \ref{fig:large_company_capped_600_bids}, the group with a capacity of 46929.2, as shown by Table \ref{table:genco_table}, bids a majority of the time the maximum bid (\textsterling600). The second most common bid is \textsterling0, or the minimum allowed bid. The bids then exponentially fall around these two peaks.
}


\DIFadd{The }\DIFaddend number of bids made by each GenCo changes dependent on the number of plants that they own, with the Orsted GenCo only making eleven bids per segment, and the largest group making 216 bids per segment \DIFaddbegin \DIFadd{(as shown by Table \ref{table:genco_table}}\DIFaddend . Figure \ref{fig:capped_600_bids} displays the uncapped scenario (\textsterling 600/MWh) and \ref{fig:capped_150_bids} displays the capped scenario (\textsterling 150/MWh).

Figure \ref{fig:large_company_capped_600_bids} shows the bids made by the largest group of GenCos as shown in Table \ref{table:genco_table}. A bimodal distribution can be seen, where the group of GenCos tend to bid either the maximum \DIFaddbegin \DIFadd{(\textsterling600) }\DIFaddend or the minimum \DIFaddbegin \DIFadd{(\textsterling0) }\DIFaddend bid. We hypothesize that they bid the maximum amount as this ensures that the market price is artificially raised, and that they are able to utilize their market power. The minimum price is bid the rest of the time to ensure that generators bids are always accepted, regardless of whether the market price has been artificially raised or not in each respective clearing segment.

Figure \ref{fig:small_company_capped_600_bids} displays the bids of the small company with a market cap of \textsterling 600/MWh. The small company also seems to have a bimodal distribution; however, \DIFdelbegin \DIFdel{bids the higher }\DIFdelend \DIFaddbegin \DIFadd{it bids the highest }\DIFaddend price more often \DIFaddbegin \DIFadd{(\textsterling600)}\DIFaddend . This may be due to the fact that it is able to influence the price at certain market segments, and the reward of the higher accepted reward outweighs the times in which it is not accepted on the market segments. Again, bidding low seems to be the strategy in which to take if the GenCo does not believe it will be able to influence the final price. As the market simulated is a uniform pricing market, having a \textsterling0 bid accepted does not mean that the GenCo will be paid \textsterling0. Rather, the GenCo will be paid the market clearing segment, which is set by the most expensive power plant accepted onto that market segment. \DIFaddbegin \DIFadd{This is the basis for a uniform pricing market, a mechanism which is often used in electricity markets. We used this mechanism in our simulation.
}\DIFaddend 
...
\centering
%\vskip\baselineskip
\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[b]{0.40\textwidth}   
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[b]{0.49\textwidth}   
\DIFaddendFL \includegraphics[width=\columnwidth]{figures/results/large_company_capped_600_bids.png}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[b]{0.49\textwidth}   
\DIFaddendFL \includegraphics[width=\columnwidth]{figures/results/small_company_capped_600_bids.png}
...
\centering
%\vskip\baselineskip
\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[b]{0.4\textwidth}   
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[b]{0.49\textwidth}   
\DIFaddendFL \includegraphics[width=\columnwidth]{figures/results/large_company_capped_150_bids.png}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[b]{0.49\textwidth}   
\DIFaddendFL \includegraphics[width=\columnwidth]{figures/results/small_company_capped_150_bids.png}
