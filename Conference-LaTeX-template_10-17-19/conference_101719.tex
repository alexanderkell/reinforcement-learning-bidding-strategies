...
\author{\IEEEauthorblockN{Alexander J. M. Kell\textsuperscript{\textsection}}
...
\footnotetext{This work was carried out whilst at Newcastle University.}
\endgroup
...
Simulation is often used as an environment for RL to learn and act in. This is due to the ability for simulation to perform actions in faster than real-time. In this section we explore RL approaches used to make intelligent bidding decisions in electricity markets. RL is a suitable method for analyzing the dynamic behaviour of complex systems with uncertainties. RL can, therefore be used to identify optimal bidding strategies in energy markets \cite{Yang2020}. In the following papers simulations are used as the environment.
...
This is the policy gradient, the gradient of the policy's performance. The policy gradient method optimizes the policy directly by updating the weights, $\theta$, in such a way that an optimal policy is found within finite time. This is achieved by performing gradient ascent on the policy and parameters $\pi^\theta$.
...
Much of the data used by the algorithm was generated by the simulation and persisted in memory. With over 4,000,000 bids made by all of the power plants in each simulation run. Each bid had a state space size of six and a single reward; leading to 28,000,000 data points per simulation. 1,000 simulation runs, therefore leads to $28\times 10^9$ data points used to train the reinforcement learning algorithm.

By modelling bidding decisions as a RL algorithm, we hope to observe the ability for RL to find the point at which market power artificially inflates electricity prices. To achieve this, we choose the six largest GenCos in the UK, as well as a smaller GenCo as a control. Groups of GenCos are modelled as a single GenCo with a single RL strategy for the purpose of this paper. These replace the case where bids are made based on the SRMC. Table \ref{table:genco_table} displays the groups of GenCos, as well as individual GenCos, with their respective capacity and number of plants.
...
For the reinforcement learning problem we have the following tuple: $(s_t,a_t,r_t,s_{t+1})$, where $(s_t, s_{t+1})$ is the state at time $t$ and $t+1$ respectively, $a_t$ is the action at time $t$, or the bid made into the market, and $r_t$ is the reward at time $t$. For our problem the state space, which are the observations given to the RL agents following each action, or bid, is given by the tuple shown in Equation \ref{eq:observation_tuple}. These observations allow the RL agents to learn how their bids should relate to this information about the market.
\begin{equation}
...
\noindent $p_m$ is the price for the $m$ load segment for the respective time step, and $M$ is the total load segments: 20. 
...
\begin{figure}
	\centering
    \includegraphics[width=0.45\textwidth]{figures/results/bounded_results.pdf}
...
\begin{figure}[]
	\centering
    \includegraphics[width=0.45\textwidth]{figures/results/unbounded_results_scatter.pdf}
...
It is noted that in the competitive market scenarios, the electricity price is close to the actual electricity price in the UK. Therefore, we can have confidence in the behaviour of our model and the RL strategy.



\begin{figure}
	\centering
    \includegraphics[width=0.45\textwidth]{figures/results/bounded_results_scatter.pdf}
...
\includegraphics[width=\columnwidth]{figures/results/large_company_capped_600_bids.pdf}
...
\hfil
\begin{subfigure}[b]{0.45\textwidth}   
\includegraphics[width=\columnwidth]{figures/results/small_company_capped_600_bids.pdf}
...
%\vskip\baselineskip
\begin{subfigure}[b]{0.45\textwidth}   
\includegraphics[width=\columnwidth]{figures/results/large_company_capped_150_bids.pdf}
...
\hfil
\begin{subfigure}[b]{0.45\textwidth}   
\includegraphics[width=\columnwidth]{figures/results/small_company_capped_150_bids.pdf}
...
\begin{figure}
	\centering
    \includegraphics[width=0.45\textwidth]{figures/results/ecdf_all.pdf}
...
\begin{figure}
\centering
    \includegraphics[width=0.45\textwidth]{figures/results/sensitivity_analysis.pdf}
    \caption{Capacity of agents using RL vs. average electricity price accepted, for bounded agents.}
...
We show that in the competitive market case, our RL agents bid a price similar to the average electricity price in the UK. This gives us confidence in the strategy of the RL algorithm. 
...
In future work, we would like to enable GenCos to withhold the capacity on offer to the electricity market. This would enable further market power by reducing competition further.  Additionally, we would like to assess the market power in different countries with different market structures and total electricity supply. In addition, we would like to run the full long-term model by using parallel computing, to see the long-term effect of market power on the market. 
